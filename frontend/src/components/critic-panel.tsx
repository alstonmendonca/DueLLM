    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 border-b border-[#F0EDE5]/8 px-4 py-2.5">
        <div className="flex h-5 w-5 items-center justify-center rounded border border-[#F0EDE5]/15">
          <span className="font-mono text-[10px] font-bold text-[#F0EDE5]/40">B</span>
        </div>
        ...
        {isStreaming && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-mono text-[10px] text-[#F0EDE5]/30">reviewing</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {!hasContent ? (
          <div className="flex h-full items-center justify-center">
            <p className="font-mono text-xs text-[#F0EDE5]/10">awaiting builder</p>
          </div>
        ) : (
          <>
            {rounds.map((r) => (
              <div key={r.round}>
                <div className="sticky top-0 z-10 flex items-center gap-2 border-b border-[#F0EDE5]/5 bg-[#312F2C]/95 px-4 py-1.5 backdrop-blur-sm">
                  <span className="font-mono text-[10px] font-medium text-[#F0EDE5]/25">
                    round {r.round}
                  </span>
                  {r.converged && (
                    <span className="rounded border border-[#F0EDE5]/20 px-1.5 py-0.5 font-mono text-[9px] font-medium text-[#F0EDE5]/60">
                      CONVERGED
                    </span>
                  )}
                </div>
